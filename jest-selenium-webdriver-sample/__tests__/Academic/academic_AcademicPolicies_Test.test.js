...
//TO:DO
/*
* Test that the scroll bar works fine on the menu
* Test that the language drop down menu works (even if English is the only option)
*
*/
...
describe("User", () => {
    ...
    it("navigates to Academic/Academic Policies", async () => {
    
      let button1 = await driver.findElement(By.partialLinkText('Academic'));
      await button1.click();
      let button2 = await driver.findElement(By.xpath("/html/body/div[1]/header/div[2]/div/div/div/div/ul/li[3]/div/ul/li[5]/a/span"));
      await button2.click();
  
      await expect(await driver.getTitle()).toBe('Academic Policies | UNIC Portal');
    }, 50000);

    it("clicks on Academic Policies tab", async () => {

      let tabBtn = await driver.findElement(By.xpath('//*[@id="policiesTabEnglish1Label"]'));
      await tabBtn.click();
  
      //let element = await driver.findElement(By.xpath('//*[@id="formsList"]/div'))
      let checkSrc = await tabBtn.getAttribute('class');
      
      await expect(checkSrc).toBe('m-nav__link m-tabs__item m-tabs__item--active');  
    }, 50000);
